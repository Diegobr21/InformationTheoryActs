...
class Heap():
    ...
    def getMin(self):
        assert(self.getType() == 'min'), ('For min value a heap with type min should be used')
        return self.heap[0]
    #Return Max Value of the list
    def getMax(self):
        assert(self.getType() == 'max'), ('For max value a heap with type max should be used')
        return self.heap[0]
    #Extracs Min Value of the list and rebuild heap
    def extractMin(self):
        assert(self.getType() == 'min'), ('For min extraction a heap with type min should be used')
        self.heap[0] = self.heap[self.size-1]
        self.heap[self.size-1] = [MAX]
        self.downheap(0)
    #Extract Max Value of the list and rebuild heap
    def extractMax(self):
        assert(self.getType() == 'max'), ('For max extraction a heap with type max should be used')
        self.heap[0] = self.heap[self.size-1]
